  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <div className="relative z-10">
        ...
        <main className="flex flex-col items-center justify-start gap-4 py-8 px-4 overflow-auto pt-28">
          {/* Title */}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none">
                      <div className="bg-black/90 backdrop-blur-sm rounded-2xl px-8 py-6 border-4 border-yellow-500 shadow-2xl">
                        <div className="flex flex-col items-center">
                          <div className={`text-6xl font-bold mb-2 ${getNumberColor(result.winningNumber) === 'red' ? 'text-red-500' :
                            getNumberColor(result.winningNumber) === 'black' ? 'text-white' :
                              'text-green-400'
                            }`}>
                            {result.winningNumber === 37 ? '00' : result.winningNumber}
                          </div>
                          <div className={`text-lg font-semibold ${getNumberColor(result.winningNumber) === 'red' ? 'text-red-300' :
                  ...
                  <div className="flex flex-col items-center gap-2 w-full">
                    ...
                    <div className="flex gap-2 flex-wrap justify-center max-w-[380px]">
                      {history.map((num, idx) => (
                        <div
                          key={idx}
                          className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-white text-sm border-2 ${getNumberColor(num) === 'red' ? 'bg-red-600 border-red-400' :
                ...
                <div className="flex gap-3 justify-center flex-wrap">
                  {CHIP_VALUES.map(chip => (
                    <button
                      ...
                      className={`w-14 h-14 rounded-full border-4 font-bold text-base shadow-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed ${chip.color
              ...
              <div className="bg-gradient-to-br from-amber-600 via-yellow-700 to-amber-700 rounded-lg p-6 shadow-2xl border-4 border-amber-900">
                  ...
                  {Array.from({ length: 36 }, (_, i) => i + 1).map(num => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative w-12 h-12 font-bold text-white text-base border-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${isRed
                ...
                <div className="flex gap-1 mb-4 justify-center">
                  <button
                    ...
                    disabled={isSpinning || busy}
                    className={`relative w-12 h-12 bg-green-600 hover:bg-green-500 text-white font-bold text-xl border-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isNumberBetOn(0) ? 'border-yellow-400 ring-2 ring-yellow-400 shadow-lg shadow-yellow-400/50' : 'border-white'
                      }`}
                  >
                    0
                    {getBetAmount(0) > 0 && (
                  ...
                  <button
                    ...
                    disabled={isSpinning || busy}
                    className={`relative w-12 h-12 bg-green-600 hover:bg-green-500 text-white font-bold text-xl border-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isNumberBetOn('00') ? 'border-yellow-400 ring-2 ring-yellow-400 shadow-lg shadow-yellow-400/50' : 'border-white'
                      }`}
                  >
                    00
                    {getBetAmount('00') > 0 && (
